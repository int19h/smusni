...
either `Live⟨state, value, trace⟩` or `Undef⟨site, trace⟩`, plus `spent`,
the traces of branches extinguished by at-issue filtering (#11 "branch-
relative representation is accepted"; MODEL_REPAIR §1).
...
`Reify c` is inert; `Holds p` evaluates the represented content at its own
occurrence with the sites fixed at formation (§5.4 row; spec :2436–2438).
...
P3. **Trace/`spent` representation**: any representation satisfying S1/S5.
...
(keyed by site/capture/handler; MODEL_REPAIR would collapse the two
probes below), `seq`/multiset (MODEL_REPAIR §1: "duplicates are two
commitments"), `occurrence-keyed set` (a set whose key includes an
evaluation-occurrence identity). The live text *leans* toward two
...
Model      : worlds W; sorted domains; world-indexed lexical interpretation;
             ...
             a PropDom with Reify : Content → PropDom, Holds : PropDom → Comp<Unit>,
             and the round-trip laws of §9.1 at the intended equality
...
Result<A>  : ⟨outcomes : P(Outcome<A>), spent : Seq<Trace>⟩
run        : Comp<A> → EvalContext → State → Result<A>
bind       : Comp<A> → (A → Comp<B>) → Comp<B>      -- per Live outcome; Undef and spent pass through
